Holm<-function(name,alpha,obj)
{
  ...
  p<-ggplot2::ggplot()+ggplot2::geom_density(data_dens,mapping=ggplot2::aes(x=rvimps),colour="darkgreen",size=1)+
    ...
    ggplot2::ggtitle(paste("Multiple testresult for",confounder,"with Holm procedure"))+ggplot2::ylab("Density")+
  ...
  res<-list(name=confounder,alpha=alpha,test=own_test,p=obj[[name]]$p,VIMP=vimp,RVIMP=rvimp,Quantil=quantile,plot=p)
}
