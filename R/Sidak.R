Sidak<-function(obj,alpha.new)
{
  ...
  quantile_low<-stats::quantile(confounder_dens,probs = alpha.new)
  ...
  p<-ggplot2::ggplot()+ggplot2::geom_density(data_dens,mapping=ggplot2::aes(x=rvimps),colour="darkgreen",size=1)+
    ...
    ggplot2::ggtitle(paste("Multiple test result for",confounder,"with Sidak procedure"))+ggplot2::ylab("Density")+
  ...
  res<-list(name=confounder,alpha=alpha.new,test=own_test,p=obj$p_dep,test_inf=own_test_low,p_inf=obj$p_inf,VIMP=vimp,RVIMP=rvimp,Quantil=quantile,plot=p)
}
