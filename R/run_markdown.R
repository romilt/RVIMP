...
#' @param Z Name of all variables the RVIMPs shall be calculated for. If \code{Z="all"} the procedure is done for all independed variables from \code{formula}.
#' @param residual.model Modeltype the residuals of \code{Z} shall be based on. Either "Linear" or "RandomForest".
#' @param reps Number of respamplings to estimate the RVIMP distribution with \link{get_distribution}.
#' @param sample.seed Seed to use for \link{get_distribution}. Must be set for reproducible results.
...
#' @param multi.test Logical. Shall a multiple test procedure be performed?
#' @param procedure If \code{multi.test=T} either \code{"Bonferroni"}, \code{"Holm"}, or \code{"Sidak"}.
...
#' @export
run_markdown <- function(formula=NULL,data=NULL,Z="all",residual.model="Linear",seed=42,reps=100,sample.seed=50,alpha.one.sided=0.05,num.trees=200,multi.test=T,procedure="Bonferroni",file.name=NULL,file.directory=NULL) {
  ...
  out<-rmarkdown::render(appDir,
                    ...
                    params = list(form=formula,dat=data,con=Z,model=residual.model,se=seed,sample_se=sample.seed,rep=reps,alpha=alpha.one.sided,num=num.trees,multi=multi.test,procedure=procedure),
