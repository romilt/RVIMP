...
plot.RVIMP<-function(x,which="all",ask=T,...){
  ...
  if((sum(!(which %in% obj$variables))>0))
  {
    if(length(which)==1)
    {
      if((which!="all"))
      {
        stop(paste("Error: Variable(s)",which[which(!(which %in% obj$variables))], "are/is not part of the RVIMP object."))
      }
    } else {
      stop(paste("Error: Variable(s)",which[which(!(which %in% obj$variables))], "are/is not part of the RVIMP object."))
    }
  ...
  if(ask==T)
  {
    if(length(which)==1)
    {
      if(which=="all")
      {
        if(length(obj$RVIMPs)==1)
        {
          ...
          ggplot2::ggplot(data = tmp,ggplot2::aes(x=name,y=VIMP,col=model))+ggplot2::geom_point()+
            ...
            ggplot2::xlab("Variable")+ggplot2::ggtitle(label = paste("Comparison for Variable",conf,"as variable of interest"))
        } else {
          graphics::par(ask=T)
          lapply(obj$RVIMPs,function(i){
            ...
            ggplot2::ggplot(data = tmp,ggplot2::aes(x=name,y=VIMP,col=model))+ggplot2::geom_point()+
              ggplot2::scale_color_discrete(name = "Model", labels = c(expression(Model[A]),expression(Model[B])))+
              ggplot2::xlab("Variable")+ggplot2::ggtitle(label = paste("Comparison for Variable",i$conf,"as variable of interest"))
          })
        }
      } else {
        ...
        ggplot2::ggplot(data = tmp,ggplot2::aes(x=name,y=VIMP,col=model))+ggplot2::geom_point()+
          ggplot2::scale_color_discrete(name = "Model", labels = c(expression(Model[A]),expression(Model[B])))+
          ggplot2::xlab("Variable")+ggplot2::ggtitle(label = paste("Comparison for Variable",obj$RVIMPs[[which]]$conf,"as variable of interest"))
      }
    } else {
      graphics::par(ask=T)
      lapply(which,function(i){
        ...
        ggplot2::ggplot(data = tmp,ggplot2::aes(x=name,y=VIMP,col=model))+ggplot2::geom_point()+
          ggplot2::scale_color_discrete(name = "Model", labels = c(expression(Model[A]),expression(Model[B])))+
          ggplot2::xlab("Variable")+ggplot2::ggtitle(label = paste("Comparison for Variable",obj$RVIMPs[[i]]$conf,"as variable of interest"))
      })
    }
  } else {
    graphics::par(ask=F)
